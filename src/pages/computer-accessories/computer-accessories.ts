...
export class ComputerAccessoriesPage {
  ...
  constructor(public navCtrl: NavController, public navParams: NavParams, public toastCtrl: ToastController, private WP: WoocommerceProvider) {
    ...
    this.WooCommerce.getAsync("products?filter[category]=accessories-laptops-computers").then( (data) => {
...
ionViewDidLoad(){
 ...
 loadMoreProducts(event){
  ...
  this.WooCommerce.getAsync("products?filter[category]=accessories-laptops-computers" + "&page=" + 
  this.page).then((data) => {
    console.log(JSON.parse(data.body));
