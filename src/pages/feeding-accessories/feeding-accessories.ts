...
export class FeedingAccessoriesPage {
  ...
  constructor(public navCtrl: NavController, public navParams: NavParams, public toastCtrl: ToastController, private WP: WoocommerceProvider) {
    ...
    this.WooCommerce.getAsync("products?filter[category]=feeding-accessories").then( (data) => {
...
ionViewDidLoad(){
 ...
 loadMoreProducts(event){
  ...
  this.WooCommerce.getAsync("products?filter[category]=feeding-accessories" + "&page=" + 
  this.page).then((data) => {
    console.log(JSON.parse(data.body));
