...
/**
 * Generated class for the SmartphonesPage page.
 *
...
export class SmartphonesPage {
  ...
  constructor(public navCtrl: NavController, public navParams: NavParams, public toastCtrl: ToastController, private WP: WoocommerceProvider) {
    ...
    this.WooCommerce.getAsync("products?filter[category]=refurbished-mobile").then( (data) => {
...
ionViewDidLoad(){
 ...
 loadMoreProducts(event){
  ...
  this.WooCommerce.getAsync("products?filter[category]=refurbished-mobile" + "&page=" + 
  this.page).then((data) => {
    console.log(JSON.parse(data.body));
