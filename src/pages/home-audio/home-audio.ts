...
export class HomeAudioPage {
  ...
  constructor(public navCtrl: NavController, public navParams: NavParams, public toastCtrl: ToastController, private WP: WoocommerceProvider) {
    ...
    this.WooCommerce.getAsync("products?filter[category]=home-audio-entertainment").then( (data) => {
...
ionViewDidLoad(){
 ...
 loadMoreProducts(event){
  ...
  this.WooCommerce.getAsync("products?filter[category]=home-audio-entertainment" + "&page=" + 
  this.page).then((data) => {
    console.log(JSON.parse(data.body));
