...
export class LaptopPage {
  ...
  constructor(public navCtrl: NavController, public navParams: NavParams, public toastCtrl: ToastController, private WP: WoocommerceProvider) {
    ...
    this.WooCommerce.getAsync("products?filter[category]=laptops-laptops-computers").then( (data) => {
...
ionViewDidLoad(){
 ...
 loadMoreProducts(event){
  ...
  this.WooCommerce.getAsync("products?filter[category]=laptops-laptops-computers" + "&page=" + 
  this.page).then((data) => {
    console.log(JSON.parse(data.body));
