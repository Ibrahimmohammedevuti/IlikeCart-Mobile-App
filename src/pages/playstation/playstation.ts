...
/**
 * Generated class for the PlaystationPage page.
 *
...
export class PlaystationPage {
  ...
  constructor(public navCtrl: NavController, public navParams: NavParams, public toastCtrl: ToastController, private WP: WoocommerceProvider) {
    ...
    this.WooCommerce.getAsync("products?filter[category]=playstaions").then( (data) => {
...
ionViewDidLoad(){
 ...
 loadMoreProducts(event){
  ...
  this.WooCommerce.getAsync("products?filter[category]=playstaions" + "&page=" + 
  this.page).then((data) => {
    console.log(JSON.parse(data.body));
